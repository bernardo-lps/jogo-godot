...
# Adicionamos '_fase = ""' para aceitar o argumento enviado pelo sinal puzzle_resolvido
func desativar_puzzle(_fase = ""):
	if puzzle_ativo != null:
		puzzle_ativo.queue_free()
		puzzle_ativo = null
		
		# Esta linha agora será executada corretamente
		$Guts.pode_mover = true
		print("Puzzle vencido! Guts liberado no World.")
